...
import { buttonStyles, customeLabelStyle, formControlStyle, typeClassNameMap } from "./styles";
  ...
  if (Array.isArray(label)) {
    return (
      ...
      <div className="flex flex-col gap-2 w-full text-wrap">
        <InputLabel sx={{ ...labelStyles, ...customeLabelStyle }}>{label[0]}</InputLabel>
...
export default function DonDangKyForm({ maDonDangKy }: DonDangKyFormProps) {
  ...
  const donDangKy = dataDonDangKy.find((item) => item.maDon.toString().trim() === maDonDangKy.trim());
  ...
  return (
    <Container className="size-fit px-8 py-6 mx-4 my-5 md:mx-0 md:min-w-[60%] lg:min-w-[50%] md:max-w-[80%]" shadow>
      <h6 className="w-full text-lg md:text-xl lg:text-2xl font-bold uppercase">{donDangKy.tenDDK}</h6>
      <FormMui
